...
sudo sysctl -p
sudo docker run -d -p 8002:8002 --security-opt seccomp=$(pwd)/chrome.json dosyago/browsergapce:2.1
# If you want sound, do below (sound is ~20 second delayed normally)
#sudo docker run -d -p 8002:8002 -d -p 8003:8003 --device /dev/snd --security-opt seccomp=$(pwd)/chrome.json dosyago/browsergapce:2.1
```
