...
export default async function Connect({port}, {adBlock:adBlock = true, demoBlock: demoBlock = false} = {}) {
  ...
  {
    const {doShot, queueTailShot} = await makeCamera(connection);
...
function getFileFromURL(url) {
  ...
  if ( ! lastNode ) {
    DEBUG.val && console.warn({fileNameError: Error(`URL cannot be parsed to get filename`)});
    return `download_${url.hostname}_${nodes.join('_')}`;
  }
